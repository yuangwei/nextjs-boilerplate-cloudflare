...
export const AuthFormCaptcha = function () {
  const { setCaptchaToken } = useFormContext();
  return (
    <Turnstile
      className="w-full"
...
export const AuthFormSubmit = function ({
  ...
  const { loading, captchaToken } = useFormContext();

  return (
    <Button
      type="submit"
      className="w-full mt-4"
      disabled={(!loading && !captchaToken) || loading}
    >
...
export const AuthFormOauthButton = function ({
  ...
  const { loading, setLoading, captchaToken } = useFormContext();
